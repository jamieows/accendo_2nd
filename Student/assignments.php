  ...
  <section class="assignments-list" aria-live="polite">
    ...
    if (empty($assignments)) {
      echo '<div class="empty-state">
              <p>No assignments found. Check back later or contact your teacher.</p>
            </div>';
    } else {
      foreach ($assignments as $a) {
        ...
        echo "<article class='assignment-card' data-status='$status' data-subject='$subject' data-title='$title'>
                <header class='card-header'>
                  ...
                  <span class='status-badge' style='background:$statusColor;' aria-label='Status: $statusLabel'>
                    $statusLabel
                  </span>
                </header>
                ...
                <div class='assignment-body'>
                  ...
                  <p class='file-requirement'>
                    File required: " . ($a['file_required'] ? 'Yes' : 'No') . "
                  </p>
                </div>

                <footer class='card-footer'>
                  <button class='voice-btn speak-btn' data-text='$title. $subject. Due $dueDate. $desc' aria-label='Read assignment details'>
                    Speak
                  </button>
                  " . ($a['submission_id'] 
                    ? ($a['grade'] !== null 
...
<style>
  ...
  @media (max-width: 768px) {
    ...
    .card-header, .card-footer { flex-direction: column; align-items: stretch; }
    .status-badge { align-self: flex-start; }
  }

  @media (prefers-contrast: high) {
    .assignment-card { border-width: 2px; }
  }
</style>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if ('speechSynthesis' in window) {
    ...
    const voice = voices.find(v => v.lang === 'fil-PH') || voices.find(v => v.lang.startsWith('en'));
